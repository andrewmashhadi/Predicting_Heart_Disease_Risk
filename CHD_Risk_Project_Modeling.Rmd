---
title: "CHD Risk Project Modeling"
output:
  html_document:
    df_print: paged
---
...
# for plotting the AUCs confidence intervals bootstrapped)
ci_subset_lr <- ci(roc_subset_lr, conf.level = 0.95)
LB_subset_lr <- ci_subset_lr[1]
mauc_subset_lr <- ci_subset_lr[2]
UB_subset_lr <- ci_subset_lr[3]

ci_elastic_lr <- ci(roc_elastic_lr, conf.level = 0.95)
LB_elastic_lr <- ci_elastic_lr[1]
mauc_elastic_lr <- ci_elastic_lr[2]
UB_elastic_lr <- ci_elastic_lr[3]

ci_pca_lr <- ci(roc_pca_lr, conf.level = 0.95)
LB_pca_lr <- ci_pca_lr[1]
mauc_pca_lr <- ci_pca_lr[2]
UB_pca_lr <- ci_pca_lr[3]

ci_xgb <- ci(roc_xgb, conf.level = 0.95)
LB_xgb <- ci_xgb[1]
mauc_xgb <- ci_xgb[2]
UB_xgb <- ci_xgb[3]

mAUC <- c(mauc_subset_lr, mauc_elastic_lr, mauc_pca_lr, mauc_xgb)
mod.id <- c("Log Reg (Subset)", "Log Reg (Elastic)", "Log Reg (PCA)", "XGBoost")
LB <- c(LB_subset_lr, LB_elastic_lr, LB_pca_lr, LB_xgb)
UB <- c(UB_subset_lr, UB_elastic_lr, UB_pca_lr, UB_xgb)

ggplot(data.frame(x = mAUC, y = mod.id, LB = LB, UB = UB), aes(mAUC, mod.id, col = mod.id)) +
  geom_point() + 
  xlab("AUC") + 
  ylab("") +
  ggtitle("Test ROC AUC Model Comparison") +
  geom_errorbar(aes(xmin = LB, xmax = UB), width = 0.2) +
  theme(axis.text = element_text(size = 12), 
        axis.title = element_text(size = 12, face = "bold"), 
        legend.position = "none") +
  geom_vline(xintercept = mauc_subset_lr, linetype = "dotted", 
                color = "black", size = 0.5) +
  theme_minimal() +
  guides(color = guide_legend(title = "Model"))

ggsave(paste0(PATH_IMAGE_DIR,"/model_AUC_comparison.png"))
```
